...
describe("mapSkillServiceError", () => {
  test("maps every service error onto its tagged API error", () => {
    const cases = [
      [new ServiceSkillNotFoundError("humanizer"), "SkillNotFoundError"],
      [new ServiceSkillDuplicateError("humanizer"), "SkillDuplicateError"],
      [new ServiceSkillNotSystemError("my-skill"), "SkillNotSystemError"],
      [
        new ServiceSystemSkillVersionMissingError("humanizer"),
        "SystemSkillVersionNotFoundError",
      ],
      [
        new ServiceSkillUpgradeInputError("humanizer", "content required"),
  ...
  test("carries the skill name and the upgrade reason across the boundary", () => {
    const duplicate = mapSkillServiceError(
      new ServiceSkillDuplicateError("humanizer")
    );
