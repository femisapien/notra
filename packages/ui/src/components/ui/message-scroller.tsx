...
function MessageScrollerButton({
  ...
  return (
    <MessageScrollerPrimitive.Button
      className={cn(
        "-translate-x-1/2 absolute inset-s-1/2 z-10 flex size-8 shrink-0 aspect-square cursor-pointer items-center justify-center rounded-full border border-black/10 bg-background/70 p-0 text-foreground shadow-[0_1px_4px_rgba(0,0,0,0.08)] backdrop-blur-sm transition-[translate,scale,opacity] duration-normal ease-emphasized hover:bg-background/90 hover:text-foreground data-[active=false]:pointer-events-none data-[active=false]:scale-95 data-[active=false]:opacity-0 data-[active=false]:duration-fast data-[active=true]:translate-y-0 data-[active=true]:scale-100 data-[active=true]:opacity-100 data-[direction=end]:bottom-4 data-[direction=end]:data-[active=false]:translate-y-full data-[direction=start]:top-4 data-[direction=start]:data-[active=false]:-translate-y-full rtl:translate-x-1/2 data-[direction=start]:[&_svg]:rotate-180 dark:border-white/15",
        className
      )}
